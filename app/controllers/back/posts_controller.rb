class Back::PostsController < BackController
  ...
  ## 面包屑导航
  add_breadcrumb Post.model_name.human, :back_posts_path
  ...
  def new
    @post = Post.new
    ## 面包屑导航
    add_breadcrumb I18n.t('view.action.new'), :new_back_post_path
  end

  # GET /posts/1/edit
  def edit
    ## 面包屑导航
    add_breadcrumb I18n.t('view.action.edit'), :edit_back_post_path
  end
  ...
  def create
    ...
    respond_to do |format|
      if @post.save
        format.html { redirect_to [:back, @post], notice: I18n.t('view.notice.created') }
  ...
  def update
    respond_to do |format|
      ...
      if @post.update(post_params)
        format.html { redirect_to [:back, @post], notice: I18n.t('view.notice.updated') }
  ...
  def destroy
    @post.destroy
    respond_to do |format|
      format.html { redirect_to back_posts_url, notice: I18n.t('view.notice.deleted') }
  ...
  def complete
    if @post.status_draft?
      ...
      redirect_to [:back, @post], notice: I18n.t("view.notice.post.completed")
    else
      redirect_to [:back, @post], alert: I18n.t("view.alert.post.status_error")
    end
  end
  ...
  def publish
    if @post.status_waiting?
      ...
      redirect_to [:back, @post], notice: I18n.t("view.notice.post.published")
    else
      redirect_to [:back, @post], alert: I18n.t("view.alert.post.status_error")
    end
  end
  ...
  def reject
    if @post.status_waiting?
      ...
      redirect_to [:back, @post], notice: I18n.t("view.notice.post.rejected")
    else
      redirect_to [:back, @post], alert: I18n.t("view.alert.post.status_error")
    end
  end
  ...
  def lock
    if @post.status_published?
      ...
      redirect_to [:back, @post], notice: I18n.t("view.notice.post.locked")
    else
      redirect_to [:back, @post], alert: I18n.t("view.alert.post.status_error")
    end
  end
  ...
  def unlock
    if @post.status_locked?
      ...
      redirect_to [:back, @post], notice: I18n.t("view.notice.post.unlocked")
    else
      redirect_to [:back, @post], alert: I18n.t("view.alert.post.status_error")
    end
  end
